...
public class SemanticChecker {
    	...
    	case "Function":
    		for(int i = 1; i < hr.getChildren().length; i++){
    			...
    			if(child.getId().equals("Assign")){
    				...
    				//isto pode estar aqui, ou √© s√≥ no assign?
				...
				if(tree.getChild(i).getId().equals("Id")){
					...
					if( lookup == null){
						if(tree.getChild(i + 1).getId().equals("ArraySize")){
							...
							symbolTable.insert(tree.getChild(i).getVal(), "int", true);			//Estas linhas s√£o um//
																								//remendo para que   //
	...
	public void checkCall(HIRTree tree, Table symbolTable){
		...
		f( io ){
		    if(funÁoes do io){
		        verificaÁoes (ja tao feitas)
		    }
		}else if (verfica se e um modulo)
		    if(verifica se e funÁao)
		         stuff needed
		else if(verifica se e funÁao deste file)
		     stuff needed
		*/
		//
